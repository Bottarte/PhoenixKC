...
public static class JoinEntityConfiguration
{
        ...
        where TRightEntity : class
    {
        ...
        )
        {
            ...
            thisBuilder.HasOne(e => e.Left).WithMany(leftWithMany).HasForeignKey(e => e.LeftId).OnDelete(deleteBehavior);
            thisBuilder.HasOne(e => e.Right).WithMany(rightWithMany).HasForeignKey(e => e.RightId).OnDelete(deleteBehavior);
        }
    }
}